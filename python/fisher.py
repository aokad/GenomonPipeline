...
def PrintHeader( myself, arg ):
    ...
    logging.info( "base_quality: {0}".format( arg.base_quality ) )
...
def construct_arguments( ):
    ...
    parser = argparse.ArgumentParser( description = "Mutation call by Fisher's exact test" )
    ...
    parser.add_argument( "out_vcf", metavar = '[VCF file]', help = "Output VCF file", type = str )
    ...
    parser.add_argument( '-q', '--map_quality', help = "Map quality threshold [30]", type = float, default = 30 )
    parser.add_argument( '-b', '--base_quality', help = "Base quality value threshold [15]", type = int, default = 15)
    parser.add_argument( '-m', '--mismatch_rate', help = "Mismatch rate threshold [0.07]", type = float, default = 0.07 )
...
def main():  
    try:
        ...
        # Pileup and fisher
        #
        logging.info( "Get pileup" )

        if ( not os.path.exists( arg.out_bam ) ):
            logging.error( "Input file: {file} not found.".format( file = arg.out_bam ) )
            raise

        pileup.Pileup_and_count( arg.out_bam, None, arg.out_vcf, arg.ref_fa, arg.base_quality, arg.mismatch_rate )
