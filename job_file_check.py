...
def main():  
    ...
    try:
        ...
        parser.add_argument( '-i', '--job_file',        help = "Input job file",    type = str )
        ...
        #
        print( "" )
        print( "##### Optional keyword check results #####" )
        ...
        if pair_id_list and -1 == job_yaml[ 'file_name' ].find( 'pair_id' ):
            print( "'file_name:' does not contain '{pair_id}'." )
            f_error |= 0x08

        if 'file_ext' in job_yaml and -1 == job_yaml[ 'file_name' ].find( job_yaml[ 'file_ext' ] ):
        ...
        dir_list = []
        if 'input_file_dir' in job_yaml and -1 == job_yaml[ 'file_name' ].find( 'pair_id' ):
            if 'sample_subdir' in job_yaml and 'file_name' in job_yaml:
                if pair_id_list:
                    for pair_id in pair_id_list:
                        dir_list.append( job_yaml[ 'input_file_dir' ] + '/' +
                                         job_yaml[ 'sample_subdir' ] + '/' +
                                         job_yaml[ 'file_name' ].format( pair_id = pair_id ) )
                else:
                    dir_list.append( job_yaml[ 'input_file_dir' ] + '/' +
                                     job_yaml[ 'sample_subdir' ] + '/' +
                                     job_yaml[ 'file_name' ] )

            if 'control_subdir' in job_yaml and 'disease_subdir' in job_yaml and 'file_name' in job_yaml:
                if pair_id_list:
                    for pair_id in pair_id_list:
                        dir_list.append( job_yaml[ 'input_file_dir' ] + '/' +
                    ...
                    dir_list.append( job_yaml[ 'input_file_dir' ] + '/' +
                                     job_yaml[ 'control_subdir' ] + '/' +
                                     job_yaml[ 'file_name' ] )
                if pair_id_list and 'file_name' in job_yaml:
                    for pair_id in pair_id_list:
                        dir_list.append( job_yaml[ 'input_file_dir' ] + '/' +
        ...
        for keyword in keywords[ 'parameters' ]:
            word = keyword.keys()[ 0 ]
            if not ( word in job_yaml ):
                for task in task_ids:
                    if keyword[ word ] in task_ids[ task ]:
        ...
        if 'project_dir_tree' in job_yaml:
            ...
            for task in task_ids:
                for nec_dir in keywords[ 'out_dir' ]:
                    if ( nec_dir.keys()[ 0 ] in task_ids[ task ] and
                         not get_dir( dir_tree, '', nec_dir.values()[ 0 ] ) ):
                        f_error |= 0x08
                        print( "{dir} is not found in 'project_dir_tree:'.".format( dir = nec_dir.values()[ 0 ] ) )
        ...
        if f_error & 0x08:
            print( "Some parameter are NOT DEFINED properly. Syntax error." )
